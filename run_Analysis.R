...
train <- read.table("UCI HAR Dataset/train/X_train.txt")[feat]
...
test <- read.table("UCI HAR Dataset/test/X_test.txt")[feat]
...
write.table(all.mean, "itssotidy.txt", row.name = F)
